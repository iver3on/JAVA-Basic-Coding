...
public class AlternateSuspendResume extends Object implements Runnable {
	...
	public static void main(String[] args) {
		...
		for ( int i = 0; i < 10; i++ ) {
			...
			//让main线程休眠450ms（>300ms），如果挂起请求发出时，
			//新线程正执行到或即将执行到stepOne操作（如果在其前面的话
			//，就会响应挂起请求，从而挂起线程），那么在stepTwo操作执行前
			//，main线程的休眠还没结束，从而main线程休眠结束后执行asr.areValuesEqual（）
			//操作进行比较时，stepTwo操作已经执行完，因此也不会出现输出结果为false的情况。
